...
function renderTribes() {
    ...
    tribesGrid.innerHTML = appData.tribal_communities.map(tribe => `
        <div class="tribe-card">
            ...
            <div class="festivals-info">
                <strong>Festivals:</strong> ${tribe.festivals.join(', ')}
            </div>
        </div>
    `).join('');
}
...
function renderHandicrafts() {
    const handicraftsGrid = document.getElementById('handicraftsGrid');
    ...
    handicraftsGrid.innerHTML = appData.handicrafts.map(craft => `
        <div class="handicraft-card">
            ...
            <div class="craft-details">
                <div><strong>Artisans:</strong> ${craft.artisans}</div>
                <div><strong>Price Range:</strong> ${craft.price_range}</div>
                <div class="availability availability--${craft.availability.toLowerCase().replace(' ', '-')}">${craft.availability}</div>
            </div>
            <button class="btn btn--primary btn--sm marketplace-link-btn">View in Marketplace</button>
        </div>
    `).join('');
